...
const standardOrder = (orderNumber: string, customerIndex: number, purchaseDate: string): OrderFixture => ({
  orderNumber,
  customerIndex,
  totalAmount: "84.00",
  purchaseDate,
...
const orders: OrderFixture[] = [
  ...
  ...Array.from({ length: 15 }, (_, index) => standardOrder(
    `WN-${1016 + index}`,
    index,
    `2026-08-${String(22 - index).padStart(2, "0")}T12:00:00.000Z`,
  )),
];
...
async function main(): Promise<void> {
  ...
  const orderRecords = new Map<string, { id: string }>();
